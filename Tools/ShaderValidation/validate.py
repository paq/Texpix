#!/usr/bin/env python3
"""Compile actual HLSL scalar helper bodies as C++ and compare a frozen oracle.

Python 3.9+, clang++ or g++. Float32/source validation, NOT Unity compilation or
GPU timing. The GLSL lab imports the extraction helpers from this module.
"""
...
def cpp_float_literals(text: str) -> str:
    # HLSL floating literals use float precision; C++ defaults to double.
...
static void check(float sample, float x, float y, float format, float mode,
    ...
    // NPOT center addressing must remain identical.
        ...
        for (int format=0; format<2; ++format) {
            ...
            for (int byte=0; byte<256; ++byte) for(int sub=0; sub<p; ++sub) {
                ...
                for (int mode=0;mode<3;++mode) for (float offset : {0.0f,0.125f,0.5f,0.875f}) {
                    ...
                    // Independent +/- one-ULP changes to interpolated thresholds.
                    const float4 prep=portable::TexpixPrepareCoverage(float2(sub+offset,0),mode,format);
                    const float4 color=legacy::TexpixShade(expected,fill,outline,mode);
                    for(float zd : {0.0f,1.0f}) for(float wd : {0.0f,1.0f}) {
                        float4 noisy=prep;
                        noisy.z=std::nextafter(prep.z,zd);
                        noisy.w=std::nextafter(prep.w,wd);
                        eq4(color,portable::TexpixShadePrepared(sample,noisy,fill,outline),"threshold jitter portable");
                        eq4(color,native_bits::TexpixShadePrepared(sample,noisy,fill,outline),"threshold jitter native");
                    }
                }
            }
        }
...
def main() -> None:
    ...
    report={'baseline_git_blob':actual,'candidate_sha256':hashlib.sha256(HEADER.read_bytes()).hexdigest(),
            'compiler':subprocess.check_output([compiler,'--version'],text=True).splitlines()[0],
            'validation_scope':'C++ translation of actual HLSL function bodies; not Unity or GPU. Contract flag is not evidence of executed FMA instructions.',
            'runs':reports}
